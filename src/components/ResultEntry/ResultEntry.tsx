...
type ResultEntryProps = { result: Geosearch; location: LatLngLocation | undefined };
  ...
  return (
    <tr className="ResultEntry--main">
      <td>
        ...
        <div>{result?.mainImage && <img src={result?.mainImage} alt="main page image" className="ResultEntry--main__image" />}</div>
      </td>
      <td
        style={{
          alignItems: "middle",
      ...
      >
        <a href={getGoogleMapLink(result)} target="_blank" className={"ResultEntry--main__google-link"}>
        ...
        <div className="ResultEntry--main__distance">
          {(result?.dist / 1000)?.toFixed(1)} <span className={"ResultEntry--main__distance--text"}>ק"מ</span>
        </div>
